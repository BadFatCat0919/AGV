...
FlagStatus KEY_RIGHT_USE_FLAG = RESET;
...
//--------【键值解析】
			...
			if(bit_istrue(SOLGUI_CSR,bit(0)))			//如果处于SPIN占用中
			{
				...
				else if(cur_key==SOLGUI_KEY_RIGHT)												//左步进
				{
					if(spin_pos<SPIN_DIGIT_MAX-1) spin_pos++;						
				}
				else if(cur_key==SOLGUI_KEY_LEFT)												//右步进
				{
					if((type==FLT16)||(type==FLT32)) { if(spin_pos>SPIN_DIGIT_MIN) spin_pos--; }	 //小数
					else { if(spin_pos>0) spin_pos--; }		//其他默认做整数处理（待扩展）
				}
			}				
			else KEY_RIGHT_USE_FLAG = RESET;
		}
//--------【选项：变量名&数值绘制】
		switch(type)
		{
			case INT8:	SOLGUI_printf(90,y_disp,F6X8,"%d",*v_i8);break;
			case UINT8:	SOLGUI_printf(90,y_disp,F6X8,"%d",*v_u8);break; 
			case INT32: SOLGUI_printf(90,y_disp,F6X8,"%d",*v_i32);break;
			case FLT16: SOLGUI_printf(90,y_disp,F6X8,"%.1f",*v_f16);break;
			case FLT32: SOLGUI_printf(90,y_disp,F6X8,"%.1f",*v_f32);break;
			case INT16: SOLGUI_printf(90,y_disp,F6X8,"%d",*v_i16);break;;	
			default:	SOLGUI_printf(90,y_disp,F6X8,"%d",*v_i16);break;
		}
